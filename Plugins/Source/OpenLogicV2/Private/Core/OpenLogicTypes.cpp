...
UOpenLogicTask* FOpenLogicTaskPool::GetTaskInstance(TSubclassOf<UOpenLogicTask> TaskClass, UObject* Outer)
{
	...
	if (AvailableTasks.Num() > 0)
	{
		...
		// Remove from root only if it is currently rooted
		if (TaskInstance->IsRooted())
		{
			TaskInstance->RemoveFromRoot();
		}
	} else
	{
...
void FOpenLogicTaskPool::ReturnTaskInstance(UOpenLogicTask* TaskInstance)
{
	...
	if (!TaskInstance->IsRooted())
	{
		TaskInstance->AddToRoot();
	}
	
	AvailableTasks.Add(TaskInstance);
}

void FOpenLogicTaskPool::CleanupPool()
{
	// Unroot all available tasks
	for (UOpenLogicTask* TaskInstance : AvailableTasks)
	{
		if (TaskInstance->IsRooted())
		{
			TaskInstance->RemoveFromRoot();
