...
void UOpenLogicRuntimeGraph::ActivateNode(TSharedPtr<FOpenLogicRuntimeNode> RuntimeNode, const FName& PinName)
{
	...
	if (TaskInstance->IsUnreachable())
	{
		UE_LOG(OpenLogicLog, Error, TEXT("[ActivateNode] TaskInstance is unreachable."));
		return;
	}
...
void UOpenLogicRuntimeGraph::SetDataPropertyValueByAddress(UOpenLogicTask* TaskInstance, FName PinName, FProperty* Property, void* SourceAddress) const
{
	...
	void* CopiedValue = FMemory::Malloc(Property->ElementSize * Property->ArrayDim);
	FMemory::Memzero(CopiedValue, Property->ElementSize * Property->ArrayDim);
...
void UOpenLogicRuntimeGraph::CleanupTaskPools(bool bSimulating)
{
	for (auto& PoolPair : TaskPools)
	{
...
void UOpenLogicRuntimeGraph::BeginDestroy()
{
	CleanupTaskPools(false);
	Super::BeginDestroy();
}
...
UOpenLogicRuntimeGraph::UOpenLogicRuntimeGraph(const FObjectInitializer& ObjectInitializer)
{
#if WITH_EDITOR
	if (GIsEditor)
	{
		FEditorDelegates::EndPIE.AddUObject(this, &UOpenLogicRuntimeGraph::CleanupTaskPools);
	}
#endif
}
